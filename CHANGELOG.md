...
- Overhauled input handling: Fixed a hard-coded limit of 4 inputs (now supports up to 20). Added the ability to set a default input on device power-on. Input name display now resets on power-off and persists during a session if changed via HomeKit.
- Cleaned up remote control defaults: Removed KEY_MUTE as the default for Volume Down Triple Press (the iOS Remote now has a dedicated Mute button) and removed KEY_STOP as the default for Play/Pause Triple Tap
- Changed default CurrentMediaState to PLAY
- Improved Config schema & README: Schema corrections, new config entries added, and fixed small README issues.
- Updated iOS and Homebridge version references in README.md

## 1.1.1 (2025-11-08)

- Updated iOS version references in README.md
- Corrected Homebridge references in README.md
